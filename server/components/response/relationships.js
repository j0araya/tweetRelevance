    ...
    _.each(data.data.relationships, function (relationship, name) {
        ...
        if (_.isArray(relationship.data)) {
            ...
            if (serverRelation.modelThrough) {
                ...
                var key = keyByModel(modelThrough, modelTo, options)
                var data = {}
                data[fkName] = id
                var stringIds = false
...
function keyByModel(assocModel, model, options) {
    var key = null
    ...
    if (key === null) {
        options.logger.error('Can not find relation for ' + model.modelName)
    }
    return key
}
